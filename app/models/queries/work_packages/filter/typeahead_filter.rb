...
class Queries::WorkPackages::Filter::TypeaheadFilter <
  ...
  def conditions_for(part)
    ...
    if (match = part.match(/\A(#)?(\d+)\z/))
      conditions << id_or_sequence_number_condition(hash_prefixed: match[1].present?, digits: match[2])
    end
  ...
  def id_or_sequence_number_condition(hash_prefixed:, digits:)
    if Setting::WorkPackageIdentifier.classic? || hash_prefixed
      id_condition(digits)
    else
      sequence_number_condition(digits)
    end
  end
