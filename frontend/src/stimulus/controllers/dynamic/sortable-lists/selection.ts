...
// it wholesale.
function describedByTokens(item:HTMLElement):string[] {
  return (item.getAttribute('aria-describedby') ?? '').split(/\s+/).filter((token) => token !== '');
}

function writeDescribedBy(item:HTMLElement, tokens:string[]):void {
  if (tokens.length === 0) {
    item.removeAttribute('aria-describedby');
  } else {
    item.setAttribute('aria-describedby', tokens.join(' '));
  }
}
...
function addDescription(item:HTMLElement, describedById:string):void {
  ...
  const tokens = describedByTokens(item);
  if (!tokens.includes(describedById)) {
    writeDescribedBy(item, [...tokens, describedById]);
  }
}
...
function removeDescription(item:HTMLElement, describedById:string):void {
  ...
  writeDescribedBy(item, describedByTokens(item).filter((token) => token !== describedById));
}
