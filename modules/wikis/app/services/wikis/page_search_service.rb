...
module Wikis
  class PageSearchService
    ...
    def build_result_tree(pages)
      ...
      pages.each_with_object({}) do |page, node_list|
        insert_wiki_node(node_list, page.wiki, root)
        insert_ancestor_nodes(node_list, page)
        insert_page_node(node_list, page)
      end
    ...
    def insert_ancestor_nodes(node_list, page)
      ...
      ancestors.reduce(previous_ancestor_node) do |previous, current|
        ancestor_node = node_list[node_key(:page, current.identifier)]
