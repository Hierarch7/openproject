...
OPENPROJECT_DEFAULT__PROJECTS__MODULES (default=["calendar", "board_view", "work_package_tracking", "gantt", "news", "costs", "wiki", "documents"]) Default enabled modules for new projects
...
OPENPROJECT_FEATURE__SUBTYPES__ACTIVE (default=false) Enables work package subtypes.
...
OPENPROJECT_FEATURE__WORK__PACKAGE__MULTIPLE__VERSIONS__ACTIVE (default=false) Enables assigning multiple (target) versions to a work package. Experimental; the user-facing setting and admin switch follow later.
...
OPENPROJECT_REAL__TIME__TEXT__COLLABORATION__ENABLED (default=true) Enable real-time collaborative editing of text fields using BlockNoteJS and Hocuspocus server.
...
OPENPROJECT_WORK__PACKAGE__MULTIPLE__VERSIONS (default=false) Enable multiple version assignments on work packages.
